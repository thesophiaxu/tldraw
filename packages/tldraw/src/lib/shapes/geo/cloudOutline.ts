...
const switchSize = <T>(size: TLDefaultSizeStyle, xs: T, s: T, m: T, l: T, xl: T) => {
	switch (size) {
		case 'xs':
			return xs
		case 's':
...
export function getCloudArcs(
	...
	const numBumps = Math.max(Math.ceil(pillCircumference / switchSize(size, 35, 50, 70, 100, 130)), 6)
